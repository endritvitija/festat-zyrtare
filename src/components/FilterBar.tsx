...
export function FilterBar({
  ...
  const handleBulkImport = (country: 'AL' | 'XK') => {
    const countryHolidays = holidays2026.filter(h => h.country === country || h.country === 'BOTH');
    downloadBulkIcsFile(countryHolidays, country === 'AL' ? 'Shqipëri' : 'Kosovë');
    setIsImportMenuOpen(false);
  };
      ...
      <div className="flex flex-wrap items-center justify-center gap-4">
        ...
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-secondary/50 hover:bg-secondary text-muted-foreground hover:text-foreground transition-all border border-transparent hover:border-border"
          >
            <Download className="w-4 h-4" />
            <span>Importo Festat</span>
            <ChevronDown className={cn("w-3 h-3 transition-transform", isImportMenuOpen && "rotate-180")} />
          </button>

          {isImportMenuOpen && (
            <div className="absolute left-0 sm:left-auto sm:right-0 mt-2 w-56 rounded-md shadow-lg bg-popover border border-border z-50 py-1 animate-in fade-in zoom-in-95 duration-100">
